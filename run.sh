...
if [[ -f "$SCRIPT_DIR/config.sh" ]]; then
    ...
    echo "[VisionTerminal] config.sh not found. Using defaults (backend=uv, local paths)."
...
case "$BACKEND" in
    uv)
        if ! command -v uv >/dev/null 2>&1; then
            echo "[VisionTerminal] 'uv' is not installed. Install it with:"
    ...
    venv)
        ...
        if [[ ! -d "$VENV_PATH" ]]; then
            echo "[VisionTerminal] Creating venv at $VENV_PATH ..."
        ...
        ;;
    *)
        echo "[VisionTerminal] Invalid VTERM_BACKEND: '$BACKEND' (use 'uv' or 'venv')."
        exit 1
        ;;
esac
